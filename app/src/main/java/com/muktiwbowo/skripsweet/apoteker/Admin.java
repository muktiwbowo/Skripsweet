package com.muktiwbowo.skripsweet.apoteker;

/**
 * Created by muktiwbowo on 29/10/17.
 */

public class Admin {
    String kasusId;
    String kasusNama;
    String penyakitNama;
    String pasienNama;
    String usiaPasien;
    String gendePasien;
    String badanBerat;
    String obatNama;
    String gejalaNama;
    String jenisHabbit;
    String solusi;

    public String getJenisHabbit() {
        return jenisHabbit;
    }

    public void setJenisHabbit(String jenisHabbit) {
        this.jenisHabbit = jenisHabbit;
    }
    public String getKasusId() {
        return kasusId;
    }

    public void setKasusId(String kasusId) {
        this.kasusId = kasusId;
    }

    public String getKasusNama() {
        return kasusNama;
    }

    public void setKasusNama(String kasusNama) {
        this.kasusNama = kasusNama;
    }

    public String getPenyakitNama() {
        return penyakitNama;
    }

    public void setPenyakitNama(String penyakitNama) {
        this.penyakitNama = penyakitNama;
    }

    public String getPasienNama() {
        return pasienNama;
    }

    public void setPasienNama(String pasienNama) {
        this.pasienNama = pasienNama;
    }

    public String getUsiaPasien() {
        return usiaPasien;
    }

    public void setUsiaPasien(String usiaPasien) {
        this.usiaPasien = usiaPasien;
    }

    public String getGendePasien() {
        return gendePasien;
    }

    public void setGendePasien(String gendePasien) {
        this.gendePasien = gendePasien;
    }

    public String getBadanBerat() {
        return badanBerat;
    }

    public void setBadanBerat(String badanBerat) {
        this.badanBerat = badanBerat;
    }

    public String getObatNama() {
        return obatNama;
    }

    public void setObatNama(String obatNama) {
        this.obatNama = obatNama;
    }

    public String getGejalaNama() {
        return gejalaNama;
    }

    public void setGejalaNama(String gejalaNama) {
        this.gejalaNama = gejalaNama;
    }

    public String getSolusi() {
        return solusi;
    }

    public void setSolusi(String solusi) {
        this.solusi = solusi;
    }
}
